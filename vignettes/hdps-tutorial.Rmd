...
The `hdps` package implements the High-Dimensional Propensity Score (HDPS) algorithm for automated covariate selection in observational studies. This tutorial demonstrates the key features and workflows of the package.
...
- **Enhanced visualizations**: Interactive plots for bias analysis
- **Flexible data input**: Support for various data formats
...
candidates <- identify_candidates(
  ...
  code = "code", 
  type = "dx",
  n = 20,  # Maximum 20 candidates
  min_patients = 5  # Minimum 5 patients per code
)

# View candidates
head(candidates$candidates)
```

## Step 2: Assess Recurrence

```{r assess-recurrence}
# Assess recurrence patterns
...
# View top covariates
head(prioritization[order(absLogBias, decreasing = TRUE)])
```
...
prioritization_parallel <- prioritize(
  cohort_data, 
  pid = "pid", 
  expo = "exposure", 
  outc = "outcome",
...
## Complete Workflow with hdps_screen
...
p1_interactive <- plot_bias_distribution(prioritization, top_n = 10, interactive = TRUE)
p1_interactive

# Interactive strength plot
p2_interactive <- plot_covariate_strength(prioritization, interactive = TRUE)
p2_interactive
```
...
1. **Use parallel processing**: Set `parallel = TRUE` in `prioritize()`
2. **Limit candidates**: Use smaller `n` values in `identify_candidates()`
3. **Filter data**: Remove rare codes before analysis
4. **Use data.table**: Ensure your data is in `data.table` format
...
3. **parallel**: Use for datasets with >1000 covariates
...
1. **Memory errors**: Reduce `n_candidates` or use parallel processing
2. **Slow performance**: Enable parallel processing
...
Schneeweiss, S., Rassen, J. A., Glynn, R. J., Avorn, J., Mogun, H., & Brookhart, M. A. (2009). High-dimensional propensity score adjustment in studies of treatment effects using health care claims data. *Epidemiology*, 20(4), 512-522.
